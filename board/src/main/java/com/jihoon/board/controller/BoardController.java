...
public class BoardController {
    ...
    @ApiOperation(value = "댓글 작성", notes = "Reqeust Header Athorization에 Bearer JWT를 포함하고 Request Body에 boardNumber, content를 포함하여 요청을 하면, 성공싱 게시물 전체  데이터를 반환함, 실패시에는 실패 메세지를 반환함")
    ...
    @ApiOperation(value = "좋아요 기능", notes = "Request Header에 Authorization에 Bearer JWT를 포함하고 Request Body에 boardNumber를 포함하여 요청을 하면, 성공시 게시물 전체 데이터를 반환, 실패시에는 실패 메세지를 반환함")
    @PostMapping(LIKE)
    ...
    @ApiOperation(value = "특정 게시물 가져오기", notes = "Path Variable에 boardNumber를 포함하여 요청을 하면 성공시 게시물 전체 데이터를 반환하고 실패시에는 실패메세지를 반환함")
    ...
    @ApiOperation(value = "전체 게시물 리스트 가져오기", notes = "요청을 하면 성공시 전체 게시물 리스트를 최신순으로 반환하고, 실패시에는 실패메세지를 반환함")
    @GetMapping(GET_LIST)
    ...
    @ApiOperation(value = "본인이 작성한 게시물 리스트 가져오기", notes = "Request Header에 Athorization에 Bearer JWT를 포함하여 요청을 하면, 성공시 요청자가 작성한 게시물 전체 리스트를 최신순으로 반환하고, 실패시에는 실패메세지를 반환함")
    @GetMapping(GET_MY_LIST)
    public ResponseDto<List<GetMyListResponseDto>> getMyList(@ApiParam(hidden=true) @AuthenticationPrincipal String email) {
        ResponseDto<List<GetMyListResponseDto>> response = boardService.getMyList(email);
        return response;
    }

    @ApiOperation(value = "검색어에 대한 게시물 리스트 가져오기", notes = "Path Variable에 searchWord와 previousSearchWord를 포함하여 요청을 하면 성공시 검색어에 해당하는 게시물 리스트를 최신순으로 반환하고, 실패시에는 실패메세지를 반환함")
    @GetMapping(value={GET_SEARCH_LIST_PREVIOUS, GET_SEARCH_LIST}) // searchWord가 현 시간 사용자가 검색하는 검색어를 말하는거고 previousSearchWord는 이전에 사용자가 검색한 검색어를 말하는건가?
    ...
    @ApiOperation(value = "검색어에 해당하는 연관 검색어 리스트 가져오기", notes = "Path Variable에 searchWord를 포함하여 요청하면, 성공시 해당하는 검색어와 관련된 검색어 중 가장 많이 검색한 15개 검색어 리스트를 반환, 실패시에는 실패 메세지를 반환함")
    ...
    @ApiOperation(value = "특정 게시물 수정", notes = "Request Header에 Authorization에 Bearer JWT를 포함하고 Request Body에 boardNumber, title, content, boardImageUrl을 포함하여 요청을 하면, 성공시에는 게시물 전체 데이터를 반환하고 실패시에는 실패메세지를 반환함")
    ...
    @ApiOperation(value = "특정 게시물 삭제", notes = "Request Header에 Authorization에 Bearer JWT를 포함하고 Path Variable에 boardNumber를 포함하여 요청을 하면 성공시 true를 반환해주고 실패시에는 실패 메세지를 반환함")
